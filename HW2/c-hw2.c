...
int main(void) {
    ...
    do {//loops entire program until you exit 
        ...
        if(game==1 && win==1)//calculates new total $
            newfunds = startfunds + (bet*7);
        else if(game==1 && loss==0)
            newfunds = startfunds - bet;
        else if(game==2 && win==1)
            newfunds = startfunds + bet;
        else if(game==2 && loss==0)
            newfunds = startfunds - bet;

        printf("\nYour total money is $%.2lf.\n", newfunds);//prints total $

        if(newfunds<=0) {//exits program if $$ falls to 0 or below
